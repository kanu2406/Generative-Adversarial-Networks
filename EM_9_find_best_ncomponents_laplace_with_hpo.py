...
experiment = get_mlflow_experiment(experiment_name="laplace_best_n_components")
...
if __name__ == '__main__':
    ...
    for c in components:
        with mlflow.start_run(run_name=f"n_components_{c}", experiment_id=experiment.experiment_id) as run:
            ...
            subprocess.run([python_executable, "EM_8_fit_Laplace_Mixture.py", "--n_components", str(c)], check=True)

            # Generate images sampled from the Gaussian Mixture
            subprocess.run([python_executable, "EM_10_laplace_generate.py"], check=True)
